function [maxlist, fmin] = max_sys_nonlnhypo(u_min, u_max, deltat, deltax, xlist, gamma)
    ...
    mesh = size(xlist);
    m = mesh(2);     
    maxlist_1 = zeros(3,m);     %for rho
    maxlist_2 = zeros(3,m);     %for momemtum
    maxlist_3 = zeros(3,m);     %for energy
    ...
    for i = 1 : 3       % 3 variables for euler equations
        ...
        for j = 2 : m - 1  %construct min function
            ...
            tgt_new = k2 - dt/dx * (fun(u_1(1), u_1(2), u_1(3), gamma) - fun(u_2(1), u_2(2), u_2(3), gamma));     %f(u_i+1/2)-f(u_i-1/2)    
